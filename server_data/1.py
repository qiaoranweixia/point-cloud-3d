...
# 6. 可视化点云（三张对比图）
def plot_three_views(points, clean_points, noise_points):
    """
    生成三张对比图：
    - 去噪前：显示原始点云（蓝色圆点）
    - 去噪后：显示去噪后的点云（蓝色圆点）
    ...
    ax1.scatter(points[:, 0], points[:, 1], points[:, 2], c='b', marker='o', s=10)
    ax1.set_title('Before Denoising')

    # 去噪后
    ax2 = fig.add_subplot(132, projection='3d')
    ax2.scatter(clean_points[:, 0], clean_points[:, 1], clean_points[:, 2], c='b', marker='o', s=10)
    ax2.set_title('After Denoising')

    # 噪声
    ax3 = fig.add_subplot(133, projection='3d')
    if len(noise_points) > 0:
        ax3.scatter(noise_points[:, 0], noise_points[:, 1], noise_points[:, 2], c='r', marker='x', s=20)
    ax3.set_title('Noise')
...
class DenoiseGUI:
    ...
    def run_denoise(self):
        ...
        # 显示三张对比图
        plot_three_views(self.points, clean_points, noise_points)
